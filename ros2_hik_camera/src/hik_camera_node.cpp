...
private:
  void declareParameters()
  {
    ...
    int auto_exposure_mode = this->declare_parameter("auto_exposure_mode", 0, param_desc);
    MV_CC_SetEnumValue(camera_handle_, "ExposureAuto", auto_exposure_mode);
    RCLCPP_INFO(this->get_logger(), "Auto exposure mode: %d", auto_exposure_mode);

    // Auto gain mode
    param_desc.description = "Auto gain mode (0: Off, 1: Once, 2: Continuous)";
    int auto_gain_mode = this->declare_parameter("auto_gain_mode", 0, param_desc);
    MV_CC_SetEnumValue(camera_handle_, "GainAuto", auto_gain_mode);
    RCLCPP_INFO(this->get_logger(), "Auto gain mode: %d", auto_gain_mode);
  }
    ...
    const std::vector<rclcpp::Parameter> & parameters)
  {
    ...
    for (const auto & param : parameters) {
      ...
      } else if (param.get_name() == "auto_exposure_mode") {
        int mode = param.as_int();
        if (mode < 0 || mode > 2) {
          result.successful = false;
          result.reason = "Invalid auto exposure mode value. Valid values are 0, 1, 2.";
        } else {
          int status = MV_CC_SetEnumValue(camera_handle_, "ExposureAuto", mode);
          if (MV_OK != status) {
            result.successful = false;
            result.reason = "Failed to set auto exposure mode, status = " + std::to_string(status);
          }
        }
      } else if (param.get_name() == "auto_gain_mode") {
        int mode = param.as_int();
        if (mode < 0 || mode > 2) {
          result.successful = false;
          result.reason = "Invalid auto gain mode value. Valid values are 0, 1, 2.";
        } else {
          int status = MV_CC_SetEnumValue(camera_handle_, "GainAuto", mode);
          if (MV_OK != status) {
            result.successful = false;
            result.reason = "Failed to set auto gain mode, status = " + std::to_string(status);
          }
        }
      } else {
        result.successful = false;
